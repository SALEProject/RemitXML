                    ...
                    if (bitSimpleSQL.Connect("DB.xml"))
                    {
                        try
                        {
                            try
                            {
                                ...
                                sql = @"SELECT TOP 1 H.*, T2R.[ID] AS [ID_Table2Report] FROM [REMIT_DataSources] DS
                                        LEFT JOIN [REMIT_DataSourceHistory] H ON (DS.[ID] = H.[ID_REMIT_DataSource])
                                        LEFT JOIN [REMIT_Table2Reports] T2R ON (H.[ID] = T2R.[ID_DataSourceHistory])
                                        WHERE (DS.[DataSourceType] = 'Table2') AND (T2R.[isSubmitted] = 1)
                                ...
                                int ID_Table2Report = Convert.ToInt32(ds_input.Tables[0].Rows[0]["ID_Table2Report"]);
                                ...
                                if (counter % 10000 == 0)
                                {
                                    ...
                                    try
                                    {
                                        Table2Report remit = new Table2Report(ID_Table2Report);
